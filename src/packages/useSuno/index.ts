...
class SunoApi {
  ...
  public async get_credits(): Promise<object> {
    ...
    console.log('get_credits response:\n', response.data);
    return {
...
/**
 * Methods:
 * - init(): Initialize the API client
 * - getClerkLatestVersion(): Get the clerk package latest version id
 * - getAuthToken(): Get the session ID and save it for later use
 * - keepAlive(isWait?: boolean): Keep the session alive
 * - generate(prompt: string, make_instrumental?: boolean, model?: string, wait_audio?: boolean): Generate a song based on the prompt
 * - generateSongs(prompt: string, wait_audio?: boolean, title?: string, tags?: string, make_instrumental?: boolean, model?: string): Generate multiple songs based on the prompt
 * - extendAudio(audioId: string, prompt?: string, continueAt?: string, tags?: string, title?: string, model?: string): Extends an existing audio clip
 * - generateLyrics(prompt: string): Generates lyrics based on a given prompt
 * - generateStems(song_id: string): Generate stems for a song
 * - getLyricAlignment(song_id: string): Get the lyric alignment for a song
 * - getClip(clipId: string): Retrieves information for a specific audio clip
 * - get_credits(): Get available credits information
 */